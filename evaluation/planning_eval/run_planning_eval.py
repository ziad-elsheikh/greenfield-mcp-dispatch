...
def print_subtask_benchmark_table(results: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    print(f"[Results Saved] Sub-task benchmark exported to {output_dir / 'subtask_eval_results.json'}")
...
def run_full_benchmark(llm: BaseChatModel, sample_size: int = 20) -> Dict[str, Any]:
    ...
    with open(output_dir / "decomposition_eval_results.json", "w", encoding="utf-8") as f:
        json.dump(summary_data, f, indent=2)
    print(f"\n[Results Saved] Benchmark output exported to {output_dir / 'decomposition_eval_results.json'}")
